...
export default function Home({ navigate }) {
  ...
  const handleDownload = () => {
    downloadMeme(memeRef.current, 'downloaded-duck-meme.png');
  };
          ...
          <div className="main-content">
              <div className="controls" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                {content.map((item, index) => (
                  <div key={index} ref={index === 0 ? memeRef : null} style={{ position: 'relative', display: 'inline-block', marginBottom: '30px' }}>
                    <img src={item.image} crossOrigin="anonymous" height="500" style={{ display: 'block' }} />
                    <div style={{
                      position: 'absolute',
                      bottom: 0,
                      left: 0,
